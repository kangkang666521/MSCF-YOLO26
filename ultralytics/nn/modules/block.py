...
class MSFE(nn.Module):
    """Multi-scale feature enhancement from Fig. 3 of MSCF-YOLO26."""

    def __init__(self, c1: int, c2: int, e: float = 0.5, shortcut: bool = True):
        """Build the shared projection, four receptive-field branches and residual path."""
        super().__init__()
        # e controls the total four-branch budget; each branch receives one quarter of it.
        hidden = max(int(c2 * e) // 4, 8)
        self.reduce = Conv(c1, hidden, 1, 1)
        self.local = Conv(hidden, hidden, 1, 1)
        self.dw3 = DWConv(hidden, hidden, 3)
        self.dw5 = DWConv(hidden, hidden, 5)
        self.dw3_d2 = DWConv(hidden, hidden, 3, d=2)
        self.fuse = Conv(hidden * 4, c2, 1, 1)
        self.refine = Conv(c2, c2, 3, 1)
        self.short = Conv(c1, c2, 1, 1, act=False) if c1 != c2 else nn.Identity()
        self.add = shortcut

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Enhance shallow details with complementary receptive fields."""
        x0 = self.reduce(x)
        y = torch.cat((self.local(x0), self.dw3(x0), self.dw5(x0), self.dw3_d2(x0)), 1)
        y = self.refine(self.fuse(y))
        return y + self.short(x) if self.add else y


class CEM(nn.Module):
    """Context enhancement module with dilated context aggregation and channel gating."""

    def __init__(self, c1: int, c2: int, e: float = 0.5, shortcut: bool = True):
        """Aggregate local, dilated and global context cues."""
        super().__init__()
        # e controls the total four-branch context budget; each branch receives one quarter of it.
        hidden = max(int(c2 * e) // 4, 8)
        self.reduce = Conv(c1, hidden, 1, 1)
        self.local = Conv(hidden, hidden, 3, 1)
        self.context = nn.ModuleList(DWConv(hidden, hidden, 3, d=d) for d in (2, 3))
        merged = hidden * (2 + len(self.context))
        squeeze = max(merged // 4, 8)
        self.pool = nn.AdaptiveAvgPool2d(1)
        self.attn = nn.Sequential(
            nn.Conv2d(merged, squeeze, 1, 1, bias=True),
            nn.SiLU(),
            nn.Conv2d(squeeze, merged, 1, 1, bias=True),
            nn.Sigmoid(),
        )
        self.fuse = Conv(merged, c2, 1, 1)
        self.short = Conv(c1, c2, 1, 1, act=False) if c1 != c2 else nn.Identity()
        self.add = shortcut

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Inject multi-range context and recalibrate channels."""
        base = self.reduce(x)
        feats = [base, self.local(base), *(m(base) for m in self.context)]
        merged = torch.cat(feats, 1)
        merged = merged * self.attn(self.pool(merged))
        merged = self.fuse(merged)
        return merged + self.short(x) if self.add else merged


class MSFECEM(nn.Module):
    """Stacked MSFE and CEM block used by the thesis feature-enhancement stage."""

    def __init__(self, c1: int, c2: int, n: int = 1, e: float = 0.5, shortcut: bool = True):
        """Apply MSFE once and refine the result with repeated CEM blocks."""
        super().__init__()
        self.msfe = MSFE(c1, c2, e=e, shortcut=False)
        self.cem = nn.Sequential(*(CEM(c2, c2, e=e, shortcut=True) for _ in range(n)))
        self.short = Conv(c1, c2, 1, 1, act=False) if c1 != c2 else nn.Identity()
        self.add = shortcut

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Enhance fine-grained details and contextual structure together."""
        y = self.cem(self.msfe(x))
        return y + self.short(x) if self.add else y


class SSFF(nn.Module):
    """Pairwise content-adaptive scale fusion from Fig. 5 of MSCF-YOLO26."""

    def __init__(self, c1: list[int] | tuple[int, ...], c2: int, target: int = 0):
        """Project features to a shared space, gate the incoming scale, then refine."""
        super().__init__()
        self.c1 = list(c1) if isinstance(c1, (list, tuple)) else [c1]
        self.target = target
        self.align = nn.ModuleList(Conv(ch, c2, 1, 1, act=False) for ch in self.c1)
        gate_hidden = max(c2 // 4, 8)
        self.gates = nn.ModuleList(
            nn.Sequential(Conv(c2 * 2, gate_hidden, 1, 1), Conv(gate_hidden, c2, 3, 1, act=False), nn.Sigmoid())
            for _ in range(max(len(self.c1) - 1, 1))
        )
        self.refine = Conv(c2, c2, 3, 1)

    def forward(self, xs: list[torch.Tensor] | tuple[torch.Tensor, ...]) -> torch.Tensor:
        """Fuse adjacent-scale features into the selected target-scale representation."""
        feats = [layer(x) for layer, x in zip(self.align, xs)]
        target_idx = self.target if self.target >= 0 else len(feats) + self.target
        y = feats[target_idx]
        gate_idx = 0
        for i, feat in enumerate(feats):
            if i == target_idx:
                continue
            if feat.shape[2:] != y.shape[2:]:
                feat = F.interpolate(feat, size=y.shape[2:], mode="nearest")
            gate = self.gates[min(gate_idx, len(self.gates) - 1)](torch.cat((y, feat), 1))
            y = y + gate * feat
            gate_idx += 1
