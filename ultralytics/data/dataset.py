    ...
    def cache_labels(self, path: Path = Path("./labels.cache")) -> dict:
        ...
        try:
            with ThreadPool(NUM_THREADS) as pool:
                consume(pool.imap(func=verify_image_label, iterable=label_iterable()))
        except PermissionError:
            LOGGER.warning(f"{self.prefix}Threaded label scanning unavailable, falling back to single-thread mode.")
            consume(map(verify_image_label, label_iterable()))
